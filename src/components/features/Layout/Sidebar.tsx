      ...
      <nav className="flex-1 overflow-y-auto p-4 space-y-6 custom-scrollbar">
        ...
        <div>
          ...
          <button
            ...
            className={`w-full flex items-center gap-3 p-3 rounded-xl mt-1 transition-colors ${
              activeView === 'daily'
          ...
          {personalCourses.map(course => {
            ...
            return (
              <button
                key={course.id}
                onClick={() => handleNavigate(course.id)}
                className={`w-full flex items-center justify-between p-3 rounded-xl border border-slate-200 dark:border-slate-700 transition-colors ${
                  activeView === course.id
